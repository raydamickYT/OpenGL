...
public:
    ...
    void Draw(unsigned int program)
    {
        ...
        assert(!indices.empty() && "No indices to draw!");
...
private:
    ...
    void setupMesh()
    {
        ...
        assert(vertices.size() * sizeof(Vertex) > 0 && "Vertex buffer is empty!");
        glBufferData(GL_ARRAY_BUFFER, vertices.size() * sizeof(Vertex), &vertices[0], GL_STATIC_DRAW);

        glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, EBO);
        assert(indices.size() * sizeof(unsigned int) > 0 && "Index buffer is empty!");
